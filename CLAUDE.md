...
- **`aisa/parse/`** — document chunking
  ...
  - `chunkers.py` — pluggable *text splitters* used by `_nemo.py::path2chunks`. Exposes `RecursiveTextChunker` (LangChain `RecursiveCharacterTextSplitter` wrapper), `LLMSemanticChunker` (LLM-driven logical chunking on small 50-token pre-pieces via the `nemo_logical-chunk` prompt, windowed with `logical_window`/`logical_stride`), and `HybridLogicalChunker` (mode `random_logical`: recursive pre-split → LLM grouping with `hybrid_window`/`hybrid_stride`, exposes `last_recursive_pieces`/`last_source_indices` so `path2chunks` can write a `-chunks.json` intermediate alongside the final `-logic-chunks.json`). `get_chunker(chunk_cfg, llm)` is the factory. **Don't confuse with `chunk.py::RecursiveChunker` — that one batches for LLM calls, these ones split text.**
...
1. **`path2chunks`** — splits markdown into token-sized chunks (`tiktoken` `gpt-3.5-turbo` encoding), stripping tables/images via `MD_PATTERNS` first. Chunker implementation is selected via `[chunking].method` (`"recursive"`, `"logical"`, or `"random_logical"`), built in `QAGenerator.__init__` via `aisa.parse.chunkers.get_chunker`. Output: `{output_dir}/doc-chunks_{size}_{method}/{doc_id}-chunks.json` (method is part of the cache path so switching methods doesn't reuse stale chunks). Mode `random_logical` additionally writes `{doc_id}-logic-chunks.json` (the final logical chunks with `source_chunk_ids` provenance) and uses that file as the cache key; `-chunks.json` in that dir holds the recursive intermediate for inspection.
